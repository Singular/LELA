...
public:	       
		  ...
		  lmodulus (p)
	{
		...
		if (modulus > (double) FieldTraits<ModularBalanced<double> >::maxModulus (max).get_d ())
		  ...
		  lmodulus (p)
	{
		...
		if ((double) modulus > (double) FieldTraits<ModularBalanced<double> >::maxModulus (max).get_d ())
			throw PreconditionFailed (__FUNCTION__, __LINE__, "modulus is too big");
	}
...
public:	  
	...
	DotProductDomain (const ModularBalanced<double> &F)
		: VectorDomainBase<ModularBalanced<double> > (F),
		  _bound ((double) ((1ULL << 53) - (int) (_F.modulus * _F.modulus)))
