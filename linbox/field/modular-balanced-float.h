...
public:	       
	...
	static ClassifyRing <ModularBalanced<float> >::categoryTag getCategory ()
		  ...
		  lmodulus (p) 
	{
		...
		if (modulus > (float) FieldTraits<ModularBalanced<float> >::maxModulus (max).get_d ())
		  ...
		  lmodulus (p)
	{
		...
		if ((float) modulus > (float) FieldTraits<ModularBalanced<float> >::maxModulus (max).get_d ())
			throw PreconditionFailed (__FUNCTION__, __LINE__, "modulus is too big");
	}

	ModularBalanced (const integer& p)
		: modulus (p.get_d ()),
		  half_mod ((p.get_d () - 1) / 2),
		  lmodulus (p.get_ui ())
	{
		if (modulus <= 1)
			throw PreconditionFailed (__FUNCTION__, __LINE__, "modulus must be > 1");
	...
	float minusOne () const { return -1.0; }
};
...
public:
	...
	FieldAXPY (const Field &F)
		: _F (F),
		  _y(0.),
		  _bound ((float) (((1ULL << 24) - (int) (_F.modulus * _F.modulus))))
		{}
...
public:	  
	...
	DotProductDomain (const ModularBalanced<float> &F)
		: VectorDomainBase<ModularBalanced<float> > (F),
		  _bound ((float) ((1ULL << 24) - (int) (_F.modulus * _F.modulus)))
